...
class roachInterface(object):
    ...
    def __init__(self):
        ...
        log_filename = conf.log_dir / (self.timestring+".log")
        self.log_filename = log_filename.as_posix()
        print(log_filename)
        ...
        # setting up ROACH
        self.divconst = 1.0 # not really necessary anymore since we only use MISTRAL-like ROACH and not OLIMPO ROACH, that needed twice the LO frequency (set to 2.0 for OLIMPO ROACH)
        ...
        port = 0
        while(not self.connect_to_valon(self, port="/dev/ttyUSB{:d}".format(port))):
            port += 1
            if port == 10:
        ...
        logging.info("Connecting to Arduino attenuators...")
        port = 0
        while(not self.connect_to_arduino(self, port="/dev/ttyACM{:d}".format(port))):
            port += 1
            if port == 10:
        ...
        self.do_double = False # what's the purpose of this?
