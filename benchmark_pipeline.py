...
class pipeline(object):
    ...
    def __init__(self, targ_path=""):
        ...
        for ii in range(nchannels):
            # find the resonance brutally as the global minimum in the mag data
            #indexmin = np.argmin(self.mag[:, ii])
            
            # find the resonance as the nearest local minimum in the mag data
            minima = argrelmin(self.mag[:, ii], order=3)
            if minima[0].size == 0: # no resonance at all -> set the midpoint
                indexmin = int(n_sweep*0.5)
            elif minima[0].size == 1: # right on the money
                indexmin = minima[0]
            else: # more than one resonance -> set the one closer to the midpoint
                distance = np.abs(minima[0]-int(n_sweep*0.5))
                argmin_distance = np.argmin(distance)
        ...
        self.centers=self.loop_centers(self.raw_chan)
